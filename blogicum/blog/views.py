...
from .const_for_blog import CONST_PAGINATE, CONST_COMMENTS_PAGINATE
...
class BlogListView(ListView):
    ...
    queryset = Post.objects.published().ordering().count_comment()
...
class Profile(ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.username == author.username:
            return posts.ordering().count_comment()
        else:
            return posts.published().ordering().count_comment()
...
class PostUpdate(PostMixin, OnlyAuthorMixin, UpdateView):
    form_class = PostForm
...
class PostDetail(ListView):
    ...
    paginate_by = CONST_COMMENTS_PAGINATE
...
class CategoryList(ListView):
    ...
    def get_object(self):
        ...
        return get_object_or_404(queryset, slug=self.kwargs['category_slug'])

    def get_queryset(self):
        return self.get_object().posts.published().ordering().count_comment()
